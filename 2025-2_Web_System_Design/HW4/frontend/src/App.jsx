...
export default function App() {
  ...
  const makeStatusBar = (state, message) => {
    // console.log("상태바 생성", message);
    setStatus(state);
    setMsg(message);
  };

  // 상태바 메시지 변경 감지 시 3초 뒤 상태바 초기화 및 사라짐
  useEffect(() => {
    if (msg === "") return;
  ...
  const handleDelete = (id) => {
    setDeleteQ((prev) => [...prev, { id, count: 3 }]);
  };
  // 삭제 대기열 변경 시 카운트 및 삭제 대상 검사.
  useEffect(() => {
    if (deleteQ.length === 0) return;
