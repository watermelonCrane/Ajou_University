...
public class Player extends Entity implements Fight {
    ...
    public int calculateDamage(Entity target) {
        ...
        double damageMag = (this.getLevel() - target.getLevel()) * 0.1 + 1.0;   //3
        damageMag = Math.max(damageMag, 0.1);   // 4

        return (int) (damage * damageMag);  // 5
    }

    // 플레이어가 데미지를 받는 메서드를 오버라이드
    ...
    public void takeDamage(int damage) {
        if (equippedArmor != null) {
            int deDamage = equippedArmor.getDefense();
            damage = Math.max(damage - deDamage, 1); // 1
